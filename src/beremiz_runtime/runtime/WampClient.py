...
from beremiz_runtime.runtime import GetPLCObjectSingleton
...
defaultWampConfig = {
    ...
    "clientFactoryOptions": {"maxDelay": 300},
    "protocolOptions": {"autoPingInterval": 10, "autoPingTimeout": 5},
}
...
class ReconnectingWampWebSocketClientFactory(
    WampWebSocketClientFactory, ReconnectingClientFactory
):
    ...
    def clientConnectionFailed(self, connector, reason):
        ...
        super(ReconnectingWampWebSocketClientFactory, self).clientConnectionFailed(
            connector, reason
        )
...
def CheckConfiguration(WampClientConf):
    ...
    if not IsCorrectUri(url):
        raise annotate.ValidateError(
            {"url": "Invalid URL: {}".format(url)}, _("WAMP configuration error:")
        )
...
def RegisterWampClient(wampconf=None, wampsecret=None):
    ...
    component_config = types.ComponentConfig(
        realm=WampClientConf["realm"], extra=WampClientConf
    )
...
registerAdapter(
    FileUploadDownloadRenderer, FileUploadDownload, formless.iformless.ITypedRenderer
)
...
webFormInterface = [
    ...
    (
        "secretfile",
        FileUploadDownload(
            label=_("File containing secret for that ID"), download_url=getDownloadUrl
        ),
    ),
    (
        "active",
        annotate.Boolean(label=_("Enable WAMP connection"), default=wampConfigDefault),
    ),
    ("url", annotate.String(label=_("WAMP Server URL"), default=wampConfigDefault)),
    (
        "clientFactoryOptions.maxDelay",
        annotate.Integer(
            label=_("Max reconnection delay (s)"), default=wampConfigDefault
        ),
    ),
    (
        "protocolOptions.autoPingInterval",
        annotate.Integer(label=_("Auto ping interval (s)"), default=wampConfigDefault),
    ),
    (
        "protocolOptions.autoPingTimeout",
        annotate.Integer(label=_("Auto ping timeout (s)"), default=wampConfigDefault),
    ),
]
...
def RegisterWebSettings(NS):
    ...
    WebSettings.addSettings(
        "wamp", _("Wamp Settings"), webFormInterface, _("Set"), wampConfig
    )
