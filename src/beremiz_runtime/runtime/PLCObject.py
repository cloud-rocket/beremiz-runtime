...
from beremiz_runtime.runtime import MainWorker, PlcStatus, default_evaluator
from beremiz_runtime.runtime.loglevels import LogLevelsCount, LogLevelsDefault
from beremiz_runtime.runtime.Stunnel import getPSKID
...
class PLCObject(object):
    ...
    def AutoLoad(self, autostart):
        ...
        try:
            self.CurrentPLCFilename = (
                open(self._GetMD5FileName(), "r").read().strip() + lib_ext
            )
    ...
    def GetLogMessage(self, level, msgid):
        ...
        if self._GetLogMessage is not None:
            ...
            sz = self._GetLogMessage(
                level,
                msgid,
                self._log_read_buffer,
                maxsz,
                ctypes.byref(tick),
                ctypes.byref(tv_sec),
            ...
            if sz and sz <= maxsz:
                return (
                    self._log_read_buffer[:sz].decode(),
                    tick.value,
                    tv_sec.value,
                    tv_nsec.value,
                )
    ...
    def _LoadPLC(self):
        ...
        try:
            ...
            self._PythonIterator = getattr(
                self.PLClibraryHandle, "PythonIterator", None
            )
            if self._PythonIterator is not None:
                self._PythonIterator.restype = ctypes.c_char_p
                self._PythonIterator.argtypes = [
                    ctypes.c_char_p,
            ...
            self._RegisterDebugVariable.argtypes = [
                ctypes.c_int,
                ctypes.c_void_p,
                ctypes.c_uint32,
            ]
        ...
        class PLCSafeGlobals(object):
            def __getattr__(self, name):
                try:
                    ...
                    raise KeyError(
                        "Try to get unknown shared global variable : %s" % name
                    )
                v = t()
            ...
            def __setattr__(self, name, value):
                try:
                    ...
                    raise KeyError(
                        "Try to set unknown shared global variable : %s" % name
                    )
        ...
        class OnChangeStateClass(object):
            def __getattr__(self, name):
                ...
                return type(
                    ...
                    (),
                    dict(
                        count=parent.python_runtime_vars[
                            "_PyOnChangeCount_" + name
                        ].value,
        ...
        try:
            ...
            for filename in filenames:
                ...
                if name.upper().startswith("RUNTIME") and ext.upper() == ".PY":
                    exec(
                        compile(
                            open(os.path.join(self.workingdir, filename), "rb").read(),
                            os.path.join(self.workingdir, filename),
                            "exec",
                        ),
                    ...
                    for methodname in MethodNames:
                        ...
                        if method is not None:
                            self.python_runtime_vars["_runtime_%s" % methodname].append(
                                method
                            )
        except Exception:
        ...
        while True:
            ...
            try:
                ...
                if exp is not None:
                    ...
                    self.LogMessage(
                        1,
                        ('PyEval@0x%x(Code="%s") Exception "%s"')
                        % (FBID, cmd, "\n".join(traceback.format_exception(*exp))),
                    )
                else:
                    res = str(result)
                ...
                res = "#EXCEPTION : " + str(e)
                self.LogMessage(
                    1, ('PyEval@0x%x(Code="%s") Exception "%s"') % (FBID, cmd, str(e))
                )
    ...
    def BlobAsFile(self, blobID, newpath):
        ...
        if blob is None:
            raise Exception(_(f"Missing data to create file: {newpath}").decode())
    ...
    def PurgePLC(self):
        ...
        old_PLC_filename = (
            os.path.join(self.workingdir, self.CurrentPLCFilename)
        ...
        if idxs:
            ...
            if self._suspendDebug(False) == 0:
                ...
                for idx, force in idxs:
                    res = self._RegisterDebugVariable(
                        idx, force, 0 if force is None else len(force)
                    )
                    if res != 0:
                        self._resumeDebug()
    ...
    def TraceThreadProc(self):
        ...
        while self.PLCStatus == PlcStatus.Started:
            ...
            res = self._GetDebugData(
                ctypes.byref(tick), ctypes.byref(size), ctypes.byref(buff)
            )
            if res == 0:
                if size.value:
