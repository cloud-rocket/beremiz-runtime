...
from formless import annotate, configurable, webform
from nevow import appserver, loaders, rend, tags, url
...
from beremiz_runtime.runtime import GetPLCObjectSingleton, MainWorker
from beremiz_runtime.runtime.loglevels import LogLevels, LogLevelsDict
...
class ISettings(annotate.TypedInterface):
    ...
    def uploadFile(
        ctx=annotate.Context(),
        uploadedfile=annotate.FileUpload(required=True, label=_("File to upload")),
    ):
        pass

    uploadFile = annotate.autocallable(
        uploadFile,
        label=_("Upload a file to PLC working directory"),
        action=_("Upload"),
    )
...
CSS_tags = [
    ...
    tags.link(
        rel="stylesheet", type="text/css", href=url.here.child("webinterface_css")
    ),
]
...
class SettingsPage(StyledSettingsPage):
    ...
    docFactory = loaders.stan(
        [
            tags.html[
                tags.head[tags.title[_("Beremiz Runtime Settings")], CSS_tags],
                tags.body[
                    tags.h1["Settings"],
...
class ExtensionSettingsPage(StyledSettingsPage):
    ...
    docFactory = loaders.stan(
        [
            tags.html[
                tags.head()[tags.title[tags.directive("title")], CSS_tags],
                tags.body[
