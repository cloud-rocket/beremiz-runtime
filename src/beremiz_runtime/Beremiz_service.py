...
from beremiz_runtime.runtime import LogMessageAndException, PlcStatus, default_evaluator
...
from beremiz_runtime.runtime.xenomai import TryPreloadXenomai
...
def version():
    print("Beremiz_service: ", __version__)
    ...
    if havewx:
        ...
        class BeremizTaskBarIcon(wx.adv.TaskBarIcon):
            ...
            def __init__(self, rpc_server):
                ...
                self.Bind(
                    wx.EVT_MENU, self.OnTaskBarChangeName, id=self.TBMENU_CHANGE_NAME
                )
                self.Bind(
                    wx.EVT_MENU,
                    self.OnTaskBarChangeInterface,
                    id=self.TBMENU_CHANGE_INTERFACE,
                )
                self.Bind(
                    wx.EVT_MENU, self.OnTaskBarLiveShell, id=self.TBMENU_LIVE_SHELL
                )
                self.Bind(
                    wx.EVT_MENU, self.OnTaskBarWXInspector, id=self.TBMENU_WXINSPECTOR
                )
                self.Bind(
                    wx.EVT_MENU, self.OnTaskBarChangePort, id=self.TBMENU_CHANGE_PORT
                )
            ...
            def CreatePopupMenu(self):
                ...
                menu.Append(
                    self.TBMENU_CHANGE_INTERFACE, _("Change IP of interface to bind")
                )
            ...
            def OnTaskBarStartPLC(self, evt):
                rt.GetPLCObjectSingleton().StartPLC()

            def OnTaskBarStopPLC(self, evt):
                rt.GetPLCObjectSingleton().StopPLC()
            ...
            def OnTaskBarChangeInterface(self, evt):
                ...
                dlg = ParamsEntryDialog(
                    None,
                    _("Enter the IP of the interface to bind"),
                    defaultValue=ip_addr,
                )
                dlg.SetTests(
                    [
                        (
                            re.compile(r"\d{1,3}(?:\.\d{1,3}){3}$").match,
                            _("IP is not valid!"),
                        ),
                        (
                            lambda x: len(
                                [x for x in x.split(".") if 0 <= int(x) <= 255]
                            )
                            == 4,
                            _("IP is not valid!"),
                        ),
            ...
            def OnTaskBarChangePort(self, evt):
                dlg = ParamsEntryDialog(
                    ...
                    defaultValue=str(self.rpc_server.port),
                )
                dlg.SetTests(
                    [
                        (str.isdigit, _("Port number must be an integer!")),
                        (
                            lambda port: 0 <= int(port) <= 65535,
            ...
            #     dlg = wx.DirDialog(None, _("Choose a working directory "), self.rpc_server.workdir,
            ...
            def OnTaskBarChangeName(self, evt):
                ...
                dlg.SetTests(
                    [(lambda name: len(name) != 0, _("Name must not be null!"))]
                )
                if dlg.ShowModal() == wx.ID_OK:
            ...
            def _LiveShellLocals(self):
                return {"locals": rt.GetPLCObjectSingleton().python_runtime_vars}
...
rt.CreatePLCObjectSingleton(WorkingDir, argv, statuschange, evaluator, pyruntimevars)
...
def FirstWorkerJob():
    ...
    rt.GetPLCObjectSingleton().AutoLoad(autostart)
...
if havetwisted and havewx:
    ...
    rt.MainWorker.interleave(waker_func, reactor.stop, FirstWorkerJob)
    ...
    ui_thread = Thread(
        target=partial(reactor.run, installSignalHandlers=False), name="UIThread"
    )
    ui_thread.start()
    ...
    try:
        ...
        rt.MainWorker.runloop(FirstWorkerJob)
    except KeyboardInterrupt:
        pass
else:
    try:
